...
const dir=require('../runtime-paths.cjs').dataDir(),config=JSON.parse(fs.readFileSync(path.join(dir,'config.json'),'utf8'));
...
async function tick(){
 ...
 try{
  ...
  await require('../worker-version.cjs').refreshRelease(dir);
  ...
  const response=await request('/workers/heartbeat',{...require('../worker-version.cjs').heartbeatVersion(dir),comfy_ready:true,tunnel_connected:!!core.confirmed,gpu_ready:local.localComfy.gpuReady,busy:local.localComfy.busy,local_url:local.localComfy.url,last_error:core.lastError?.slice(0,1000)||null},credential.device_token);
